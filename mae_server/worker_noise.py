...
启动方式（在项目根目录下）：
    uvicorn worker_noise:app --host 0.0.0.0 --port 28011
...
sys.path.append(str(Path(__file__).parent / "privacy_cam_attack_main0615"))
from privacy_cam_attack_main0615.scripts.make_manual_recon_grid import ManualReconTool

# ====================== CONFIG ======================
# 使用的 GPU ID 列表，指定几个就启动几个模型实例
# 例如 [0, 1, 2, 3] 表示在 cuda:0/1/2/3 上各加载一个模型，并发推理
GPU_IDS: list[int] = [7]

# 模型权重路径
CKPT_PATH: str = "/workspace/s/yuanc/jzy/VideoCam/mae/models/P_best_0622.pt"

# 是否启用 AMP（自动混合精度）加速
USE_AMP: bool = True

# 服务监听端口
SERVER_PORT: int = 28011

# 重建模型相关固定参数
RECON_CONFIG = {
    "encoder": "privacy",
    "privacy_feature_mode": "direct_noisy",
    "vision_dp_epsilon": 50,
    "vision_dp_noise_location": "vit_hidden_25",
    "vision_dp_noise_type": "aGM",
    "reconstructor_type": "mae",
}
# =====================================================
...
class WorkerPool:
    """
    多 GPU Worker 池：每个 GPU 加载一个 ManualReconTool 实例；
    任务按"当前在途请求数最少"分派到不同 GPU，实现负载均衡。
    同一 GPU 上的请求通过 pending 计数互斥（避免多线程争用同一 CUDA 上下文）。
    """

    def __init__(self):
        self.workers: list[ManualReconTool] = []
        self.devices: list[str] = []
        self.pending: list[int] = []
        self._lock = threading.Lock()

    def add(self, worker: ManualReconTool, device: str):
        self.workers.append(worker)
        self.devices.append(device)
        self.pending.append(0)

    def acquire(self) -> tuple[int, ManualReconTool]:
        with self._lock:
